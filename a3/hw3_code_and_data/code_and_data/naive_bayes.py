...
def train_mle_estimator(train_images, train_labels):
    ...
    t = np.sum(train_labels, axis=0)
    tx = np.dot(np.transpose(train_images), train_labels)
    theta_mle = tx / t
    pi_mle = t / len(train_labels)
...
def log_likelihood(images, theta, pi):
    ...
    sum_pi_x_theta = np.log(pi) + np.dot(images, np.log(theta)) \
                      + np.dot((1-images), np.log((1-theta)))
    log_sum = np.log(np.sum(np.exp(sum_pi_x_theta), axis=1)).reshape(len(sum_pi_x_theta), 1)
    log_like = sum_pi_x_theta - log_sum
    return log_like
