...
def evaluate(targets, y):
    ...
    #####################################################################
    n = len(targets)
    temp1 = - np.dot(np.transpose(targets), np.log(y))
    temp2 = - np.dot(np.transpose((1-targets)), np.log(1-y))
    ce = (temp1 + temp2) / len(targets)
    ce = ce[0][0]
    correct = 0
    for i in range(n):
        if (targets[i]==1 and y[i]>=0.5) or (targets[i]==0 and y[i]<0.5):
            correct += 1
...
def logistic(weights, data, targets, hyperparameters):
    ...
    df = np.dot(np.transpose(data), y-targets) / data.shape[0]
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return f, df, y
